...
{	
    ...
    {    	
    	...
    	$this->render('show', compact('packages'));
    }
    ...
    {   	
    	...
    	//$services = PackageService::model()->findAll(array('order'=>'order_index'));    	
    	//pakiety wraz z przypisanymi usługami    	
    	//$packagesData = Package::model()->findAll(array('order'=>'order_index desc'));    	
    	...
    	$this->render('comparison', compact('services', 'packages'));   	    	
    }
...
//        $packageTest = Package::model()->findByPk(5);   
//        var_dump($packageTest);
//        echo '<br>'.$packageTest->active;
//        echo '<br>'.$packageTest->creators;
    	if (isset($_POST['PackagePurchase'])) {    		
            $purchase->attributes = $_POST['PackagePurchase'];
            
//            echo '<br>'.$purchase->creators;
            ...
            if ($purchase->validate()) {
                ...
                try {
                    ...
                    {					
                        ...
                        array('and', 'user_id='.$purchase->user_id, 'status='.Package::$_packagePurchaseStatus['PURCHASE_STATUS_CURRENT'], 'creators'));
                    }  	    			
                    $purchase->save();
                    ...
                    if($purchase->status == Package::$_packagePurchaseStatus['PURCHASE_STATUS_CURRENT']) {
                        ...
                        User::model()->updateByPk($purchase->user_id, array(
                                    ...
                                    'package_expire' => $purchase->date_expire				    
                                ));
                        if($purchase->package_id != Yii::app()->user->package_id)
                        {			        	            
                                        // update items package cache
                        Yii::app()->db->createCommand()
                            ->update('tbl_item', array(
                                'cache_package_id'=>$purchase->package_id
                            ), 'user_id=:user_id', array('user_id'=>$purchase->user_id));
                    ...
                    if($purchase->status == Package::$_packagePurchaseStatus['PURCHASE_STATUS_CURRENT'])
                    {
                        Yii::app()->user->setState('package_id', $purchase->package_id);    
...
//        $packages = Package::model()->findAll(array('condition'=>'active', 'order'=>'order_index desc'));    
    	$this->render('change', compact('purchase', 'packages', 'canTest', 'articleRight1', 'articleRight2'));
    }
     ...
     */
	public function actionTransactionConfirm_test_notused()
    {
	...
	public function actionTransactionConfirm()
    {
    	...
    	try {
	    	...
	    	{		    		
	    		...
	    		if ($purchaseUserData['package_id'] != $pendingPurchase['package_id'])
		    		Yii::app()->db->createCommand()
                ...
                Yii::app()->mailer->systemMail(
                    ...
                    $this->render('transactionConfirmEmail',
                        array('user' => $purchaseUserData), true),
                    null,
                    true,
    ...
    public function actionDisablePurchasedPackages_old_20150518()
    {
    	$viewContent = '';
    	$viewContent .= '<br/>Operacaja CRONa - wygaszanie i uruchamianie pakietów !!!<br/>';
    	/*
	...
	public function actionNotifyPackageExpireAfter_old_20150518()
    {
    	$viewContent = '';
    	$viewContent .= '<br/>Operacaja CRONa - powiadomienia o wygaśnięciu pakietu !!!<br/>';
		...
		for($i=0;$i<count($expireAfter);++$i)
		{
			...
			foreach($data as $row)
			{
				$viewContent .= '<br/>-- Użytkownik - id:'.$row['user_id'].', nazwa:'.$row['username'].', email:'.$row['email'];
				$viewContent .= '<br/>--- wygasa w dniu: '.$row['package_expire'].', id zamówienia: '.$row['id'];
				//sprawdzenie czy istnieje opłacone zamówienie dla użytkownika, jeśli tak to nie ma sensu go informowac
				if(PackagePurchase::model()->find(
					array(
    ...
    public function actionDisablePurchasedPackages_20191201()
    {
    	$viewContent = '';
    	$viewContent .= '<br/>Operacaja CRONa - wygaszanie i uruchamianie pakietów !!!<br/>';    	
    	/*
