...
<?php
	...
	//print_r($user->packageCreators);
	//echo 'id'.Yii::app()->params['packages']['dotpayId'];
	$packageCurrent = $user->packageCreators;
    ...
    $packageLast = User::model()->getLastPackage(Yii::app()->user->id, true);
	
    if(!isset($packagePaid))
		$packagePaid = PackagePurchase::model()->exists('user_id=:user_id and status=:status and creators', array(':user_id'=>Yii::app()->user->id, ':status'=>Package::$_packagePurchaseStatus['PURCHASE_STATUS_PAID']));
    ...
    if($packageLast && ($packageLast['purchase']['status'] == Package::$_packagePurchaseStatus['PURCHASE_STATUS_PENDING']))
    {
		...
		$dotpayForm .= CHtml::hiddenField('URLC', $this->createAbsoluteUrl('packages/transactionconfirm/id/'.$packageLast['purchase']['id']));
    ...
    $tdvAttributes[] = array(
    		...
    		.(Yii::app()->controller->action->id != 'change' ?
    				...
    				(!$packagePaid && $packageCurrent['id'] != Yii::app()->params['packages']['defaultPackageCreators'] ?  '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary" href="'.$this->createUrl('packages/change/package/'.$packageCurrent['id']).'">'.Yii::t('packages', 'Extension of the package').'</a>':'')
	 ...
	 */
	if($packageCurrent['id'] != Yii::app()->params['packages']['defaultPackageCreators'])
	{
		$tdvAttributes[] = array(
			'value'=> $user['creators_package_expire'] ? Yii::app()->dateFormatter->format("yyyy-MM-dd", $user['creators_package_expire']) : Yii::t('packages', 'No limit'),
			//'value'=> $packageCurrent['id'] != Package::$_packageDefault && $packageCurrent['purchase']['date_expire'] ? Yii::app()->dateFormatter->format("yyyy-MM-dd", $packageCurrent['purchase']['date_expire']) : Yii::t('packages', 'No limit'), 
	...
	{
		$tdvAttributes[] = array(
			'value'=> ($packageCurrent['id']==$packageLast['id'] ? Yii::t('packages', 'Extension of the package:') : Yii::t('packages', 'Package change to:'))
				.'&nbsp;&nbsp;&nbsp;'
				.Package::badge($packageLast['name'], $packageLast['css_name']).' / '.$packageLast['purchase']['period'].' '.Yii::t('packages', 'months')
				.($packageLast['purchase']['status'] != Package::$_packagePurchaseStatus['PURCHASE_STATUS_PAID'] ?
				'  <span style="color:red; margin-left:10px;">- '.Package::statusToLabel($packageLast['purchase']['status'], $packageLast['purchase']['paid']).' </span>'
				...
				:''), 
			'label'=> Yii::t('packages', 'Order in progress').':',
