...
//if($canTest) {
	...
	foreach($packagesToSelect as $package) {
		// periodsData for package
		$periodsData  = array();
		foreach($package['periods'] as $period) {
			$periodsData [$period['period']]= array(
				//'period' => $period['period'],
				'price' => $period['price'] . ' PLN'
			);
		}

		// package data
		$packagesData [$package['package']['id']]= array(
			'test_period' => $package['package']['test_period'],
			'test_label' => Yii::t('packages', 'Test ({period} days)', array('{period}'=> $package['package']['test_period'])),
			'periods' => $periodsData
		);
	}
    ...
    <div class="span6">        
        <?php $form = $this->beginWidget('ActiveForm', array(            
			...
			<?php echo $form->dropDownListRow(
				$purchase,
				'package_id',
				Package::packagesToSelect(false),
				array('options' => array($selectedId=>array('selected'=>true)))
			); ?>
			<?php echo $form->dropDownListRow(
				$purchase,
				'period',
				array()//PackagePeriod::periodsToSelect()
			); ?>
			<?php echo $form->textFieldRow($purchase, 'price', array(
				"disabled"=>"disabled"
			)); ?>
			<?php 
